defmodule ScaleGenerator do
  ...
  def step(scale, tonic, step) do
    index = scale |> Enum.find_index(&(&1 === tonic))

    Enum.at(scale, @steps[step] + index - 13)
  end
  ...
  def chromatic_scale(tonic \\ "C"), do: Scale.scale(tonic)
  ...
  def flat_chromatic_scale(tonic \\ "C"), do: Scale.flat_scale(tonic)
  ...
  def find_chromatic_scale(tonic) when tonic in ~w(F Bb Eb Ab Db Gb d g c f bb eb), do: flat_chromatic_scale(tonic)
  ...
  def scale(tonic, pattern) do
    scale_ = find_chromatic_scale(tonic)
    tonic = String.capitalize(tonic)

    pattern
    |> String.graphemes()
    |> Enum.reduce([tonic], fn step, [h | t] ->
      [step(scale_, h, step) | [h | t]]
    end)
    |> Enum.reverse()
  end
end
