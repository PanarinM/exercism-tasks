defmodule ScaleGeneratorTest do
  ...
  describe "step to next note" do
    test "with half-tone interval" do
      assert ScaleGenerator.step(~w(C C# D D# E F F# G G# A A# B C), "C", "m") == "C#"
    end

    test "with full tone interval" do
      assert ScaleGenerator.step(~w(C C# D D# E F F# G G# A A# B C), "C", "M") == "D"
    end

    test "with accidental interval" do
      assert ScaleGenerator.step(~w(C C# D D# E F F# G G# A A# B C), "C", "A") == "D#"
    end
  end
